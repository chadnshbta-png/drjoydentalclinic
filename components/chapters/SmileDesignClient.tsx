...
import ConsultationReveal from "@/components/craft/ConsultationReveal";
import styles from "./SmileDesign.module.css";

/**
 * "Let's Design Your Perfect Smile" — a guided digital consultation, not a
 * form. Text and controls live on the left; a single treatment visual lives on
 * the right and, on "Reveal my treatment", crossfades from the selected
 * treatment's Before render to its After render. Clean zones, generous space,
 * one idea per screen.
 */

type SmileStep = "choose" | "reveal" | "journey" | "result";
...
interface Choice {
  ...
  sub: string;
  color: string;
  /** folder in public/A Personal Consultation for this treatment's before/after */
  slug: string;
  condition: string;
  treatment: string;
  duration: string;
...
const CHOICES: Choice[] = [
  ...
  { id: "pain", label: "Restorative Care", sub: "Repair & renew", color: "#957846",
    slug: "Digital Dentistry",
    condition: "A tooth asking for care", treatment: "Digitally-Planned Restoration",
    duration: "1–3 visits", appts: "As needed", comfort: 5,
    outcome: "Function and beauty restored — planned in a precise digital twin." },
  { id: "preventive", label: "Alignment & Health", sub: "Straighten & protect", color: "#D6B882",
    slug: "Orthodontics",
    condition: "A bite ready for harmony", treatment: "Orthodontic Alignment",
    duration: "12–24 months", appts: "Every 6–8 weeks", comfort: 4,
    outcome: "A balanced, lasting bite — health and beauty in step." },
];
...
export default function SmileDesignClient({
  pairs,
}: {
  /** before/after pairs keyed by treatment slug (public/A Personal Consultation) */
  pairs: Record<string, Service>;
}) {
  ...
  const pair = choice ? pairs[choice.slug] ?? null : null;
  ...
  return (
    <section id="smile" ref={rootRef} className={styles.smile}>
      ...
      {/* ——— right: the treatment before/after visual ——— */}
      <div className={styles.canvasCol}>
        <ConsultationReveal pair={pair} revealRef={revealRef} active={live} />
      </div>
    </section>
  );
}
